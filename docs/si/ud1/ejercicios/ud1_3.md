# UD1.3 · Rutas, ficheros y directorios
...
![Organización esperada de UD1.3](../../../assets/images/si/ud1_3_workspace.svg)
...
| Ruta | Uso |
|---|---|
| `ud1_3.sh` | Script principal. Contendrá el menú y las operaciones del ejercicio. |
| `scripts/crear_estructura_ud1_3.sh` | Genera la estructura inicial sobre la que vas a trabajar. |
| `scripts/reiniciar_ud1_3.sh` | Permite borrar la prueba y volver a crearla para repetir el ejercicio. |
| `pruebas/ud1_3/` | Zona de pruebas. Aquí se crean, copian, mueven y eliminan los ficheros. |
...
```text
/workspace/ud1/scripts/crear_estructura_ud1_3.sh
```

Este script debe crear automáticamente la siguiente estructura dentro de `/workspace/ud1/pruebas/ud1_3`:
...
```bash
bash /workspace/ud1/scripts/crear_estructura_ud1_3.sh
```

y comprobar el resultado con:

```bash
tree /workspace/ud1/pruebas/ud1_3
```
...
```text
/workspace/ud1/scripts/reiniciar_ud1_3.sh
```

Su objetivo será dejar el ejercicio preparado para empezar otra vez:

1. Eliminar `/workspace/ud1/pruebas/ud1_3` si existe.
2. Ejecutar `crear_estructura_ud1_3.sh` para volver a generar la estructura inicial.
...
La opción **2** debe ejecutar el script auxiliar `reiniciar_ud1_3.sh` y la opción **3** debe mostrar con `tree` el estado actual de la zona de pruebas.

## 5. Función `mostrar`

Dentro de `ud1_3.sh`, crea una función llamada `mostrar` que reciba **un argumento** con la descripción de la última operación realizada.

Cada vez que se invoque debe:

1. Limpiar la pantalla.
2. Mostrar `OPERACIÓN:` seguido del argumento recibido.
3. Mostrar la estructura actual de `/workspace/ud1/pruebas/ud1_3` mediante `tree` y una **ruta absoluta**.
...
```text
/workspace/ud1/pruebas/ud1_3/java/ViewModels
```
...
```text
/workspace/ud1/pruebas/ud1_3/java/views/panels
```

Desde esa ubicación utiliza una **ruta relativa** para crear `operations` directamente dentro de `ud1_3`.
...
Desplázate hasta `ud1_3` y, utilizando **rutas relativas**, copia `java/config` y todo su contenido dentro de `operations`.
...
```text
/workspace/ud1/pruebas/ud1_3/resources/img
```
...
- que `crear_estructura_ud1_3.sh` genera correctamente el escenario inicial;
- que `reiniciar_ud1_3.sh` permite repetir el ejercicio;
